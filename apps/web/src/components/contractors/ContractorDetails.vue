<template>
    <v-card>
        <v-card-title class="text-h6">Szczegóły kontrahenta</v-card-title>
        <v-card-text v-if="!contractor">
            <v-alert type="info" variant="tonal">Nie wybrano kontrahenta.</v-alert>
        </v-card-text>
        <v-card-text v-else>
        ...
        <v-card-actions>
            ...
            <v-btn color="error" variant="text" :disabled="!contractor" :loading="deleting" @click="handleDelete">
                Usuń
            </v-btn>
            <v-btn color="#d63031" :disabled="!formValid || !contractor" :loading="saving" @click="submit">
                Zapisz
            </v-btn>
        </v-card-actions>
    </v-card>
...
import type { IContractor, IContractorUpdateBody } from "../../api";
...
const emit = defineEmits<{
    (e: "update", payload: IContractorUpdateBody): void;
    (e: "delete", contractor: IContractor): void;
    (e: "close"): void;
}>();
...
const submit = () => {
    if (!formValid.value || !props.contractor) return;
    emit("update", {
...
const handleDelete = () => {
    if (!props.contractor) return;
    emit("delete", props.contractor);
};
</script>
